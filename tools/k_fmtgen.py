...
def conv_feature(conv):
    """Feature group a conversion belongs to (for --disable enforcement)."""
    if conv.kind == "LIT":
        return None
    if conv.kind == "STR":
        return "str"
    ...
    if conv.spec == "p":
        return "ptr"
    return {BASE_DEC: "dec", BASE_HEX: "hex",
            BASE_OCT: "oct", BASE_BIN: "bin"}[conv.base]


def generate(messages, outdir, seed=0xC0FFEE, n_random=8, disabled=frozenset()):
    ...
    for sym, fmt in messages:
        ...
        for c in convs:
            feat = conv_feature(c)
            if feat is not None and feat in disabled:
                raise FmtError("%s: uses disabled feature '%s' (%%%s)"
                               % (sym, feat, c.spec))
            if c.kind == "LIT":
                a = intern_lit(c.text)
    ...
    _write_hw_header(outdir, msg_meta)
...
def _write_hw_header(outdir, meta):
    """C bridge for softcore clients driving the kp_regs register window:
    message ids + arity table + register offsets + an inline write-to-fire
    sender. The softcore never carries the format strings - printf with the
    formats compiled out of flash entirely."""
    with open(os.path.join(outdir, "k_printf_hw.h"), "w") as f:
        f.write("/* GENERATED by tools/k_fmtgen.py - do not edit. */\n")
        f.write("#ifndef K_PRINTF_HW_H\n#define K_PRINTF_HW_H\n\n")
        f.write("/* kp_regs register map (32-bit word offsets) */\n")
        f.write("#define KP_HW_REG_ARG(n)  (n)\n")
        f.write("#define KP_HW_REG_SEND    8\n")
        f.write("#define KP_HW_REG_STATUS  9\n")
        f.write("#define KP_HW_STATUS_PEND 0x1u\n")
        f.write("#define KP_HW_STATUS_ERR  0x2u\n\n")
        f.write("/* message ids (KP_ISA_VERSION %d) */\n" % KP_ISA_VERSION)
        f.write("enum kp_hw_msg {\n")
        for mid, (sym, fmt, argtypes) in enumerate(meta):
            f.write("    KP_MSG_%s = %d,\n" % (sym.replace(BAD_UOP_SYMBOL, "TEST_BAD_UOP")
                                               if sym == BAD_UOP_SYMBOL else sym, mid))
        f.write("};\n\n")
        f.write("static const unsigned char kp_hw_arity[%d] = {\n    " % len(meta))
        f.write(", ".join(str(len(a)) for (_, _, a) in meta))
        f.write("\n};\n\n")
        f.write("/* Poll-then-fire (the register-window STALL contract): waits for\n"
                " * the previous SEND to be accepted, writes the args, then fires\n"
                " * atomically by writing the id to SEND. */\n")
        f.write("static inline void kp_hw_send(volatile unsigned int *base,\n"
                "                              unsigned msg_id,\n"
                "                              const unsigned int *args,\n"
                "                              unsigned nargs)\n{\n"
                "    unsigned i;\n"
                "    while (base[KP_HW_REG_STATUS] & KP_HW_STATUS_PEND) { }\n"
                "    for (i = 0; i < nargs; i++)\n"
                "        base[KP_HW_REG_ARG(i)] = args[i];\n"
                "    base[KP_HW_REG_SEND] = msg_id;\n"
                "}\n\n")
        f.write("#endif /* K_PRINTF_HW_H */\n")
...
def main():
    ...
    ap.add_argument("--disable", default="",
                    help="comma list of feature groups to reject "
                         "(dec,hex,oct,bin,ptr,str,chr) - the K_PRINTF_ENABLE_* "
                         "analogue; a message using one fails generation")
    args = ap.parse_args()
    disabled = frozenset(x.strip() for x in args.disable.split(",") if x.strip())
    bad = disabled - {"dec", "hex", "oct", "bin", "ptr", "str", "chr"}
    if bad:
        print("k_fmtgen: error: unknown --disable group(s): %s" % ", ".join(sorted(bad)),
              file=sys.stderr)
        return 2
    try:
